-- Excel Course - Complete Reset Script
-- WARNING: This deletes ALL Excel course modules, lessons, and quizzes, then recreates everything fresh

-- Step 1: Get the Excel course ID
...
-- Step 2: Delete all modules, lessons, quizzes for this course (foreign key will cascade)
DELETE FROM modules WHERE course_id = @excel_course_id;

-- Step 3: Verify modules are deleted
SELECT COUNT(*) AS remaining_modules FROM modules WHERE course_id = @excel_course_id;

-- ============================================
-- RECREATE ALL MODULES AND LESSONS FROM SCRATCH
-- ============================================
...
(@module_id, 'Advanced Logical Functions', 'advanced-logical-functions', 'Master complex logical functions.', 'Master complex logical functions.', 'https://youtube.com/watch_v=example58', 1200, 4, 0, 1),
...
-- Count modules
SELECT COUNT(*) AS total_modules FROM modules WHERE course_id = @excel_course_id;
...
-- Show all modules with lesson counts
SELECT 
    m.id,
    m.title,
...
SELECT 'Excel course reset complete! No more duplicates.' AS result;
